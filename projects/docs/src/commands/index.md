...
`cradle doctor` checks `pi` (required), and `nono`/`sbx`/`mise` (recommended — each on your `PATH`, with version). `cradle start` reads the agent folder and launches `pi` in your current working directory — sandboxed inside `nono` or `sbx` when the folder declares `sandbox/nono.json`/`sandbox/sbx.json`, bare (with a warning) otherwise.

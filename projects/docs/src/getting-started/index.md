...
| [`nono`](https://github.com/nolabs-ai/nono)  | Recommended | The filesystem sandbox pi runs inside for a nono-backend run — a folder declaring `sandbox/nono.json`, or `--sandbox`/`--sandbox-backend nono`/`--offline`/`--allow-host`. Required for that run, not needed otherwise.                                                             |
